...
class mhtProducer(Module):
    ...
    def analyze(self, event):
        ...
        for i, j in enumerate(jets):
            if not self.jetSel(j):
                continue
            if j.muonIdx1 != -1 and j.muonIdx1 < njets:
                if self.muSel(muons[j.muonIdx1]):
                    continue  # prefer the muon
            if j.muonIdx2 != -1 and j.muonIdx2 < njets:
                if self.muSel(muons[j.muonIdx2]):
                    continue  # prefer the muon
...
mht = lambda: mhtProducer(lambda j: j.pt > 40, lambda mu: mu.pt > 20 and mu.
                          miniPFRelIso_all < 0.2, lambda el: el.pt > 20 and el.
